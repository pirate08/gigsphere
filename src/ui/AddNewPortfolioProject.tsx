...
interface AddNewPortfolioProps {
  isOpen: boolean;
  onClose: () => void;
  // Function to handle saving the new experience
  onSave: (newCertificate: PortfolioItem) => void;
}

const AddNewPortfolioProject: React.FC<AddNewPortfolioProps> = ({
  isOpen,
  onClose,
  onSave,
}) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Basic validation
    if (!name.trim()) {
      setError(' Name of the project is required.');
      return;
    }
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70 backdrop-blur-sm p-4'>
      ...
      <div className='bg-gray-800 border border-gray-700 rounded-xl shadow-2xl w-full max-w-lg max-h-[100vh] p-6 sm:p-8 transform transition-all duration-300 scale-100'>
        ...
        <form onSubmit={handleSubmit} className='space-y-5'>
          ...
          {/* --Url-- */}
          <div>
            <label htmlFor='url' className='block text-sm text-gray-300 mb-1'>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder='Shortly describe your project under 1000 words...'
              required
            />
          </div>
...
export default AddNewPortfolioProject;
